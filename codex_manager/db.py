...
def list_threads(state_db, limit=15):
    ...
    header = f"{'#':<3} | {'Provider':<8} | {'Model':<14} | {'Name':<16} | {'Status':<8} | {'Thread ID':<36} | {'Title'}"
    print(header)
    print('-' * len(header))
    for idx, r in enumerate(rows, 1):
        ...
        print(f"{idx:<3} | {prov or 'unknown':<8} | {mdl or 'unknown':<14} | {nm:<16} | {status:<8} | {tid} | {t_clean}")
...
def get_paired_threads(state_db):
    ...
    cur = conn.cursor()
    rows = cur.execute(
        "SELECT id, name, title, model_provider, archived, updated_at FROM threads"
    ).fetchall()
    conn.close()
