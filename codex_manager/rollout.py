...
def make_wire_record(rec_type, payload, ord_num):
    ...
    return json.dumps(
        {"timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ"), "ordinal": ord_num, "type": rec_type, "payload": payload},
        ensure_ascii=False,
    )


def build_migrated_rollout_file(
    source_rollout, new_thread_id, new_model, target_provider, clean_cwd, turns, out_rollout_path
):
    """
    ...
    for t in turns:
        ...
        add_rec("turn_context", {"turn_id": tid, "root_turn_id": tid, "cwd": clean_cwd, "model": new_model})
        ...
        add_rec(
            "event_msg",
            {
                ...
                "turn_id": tid,
                "item": {"type": "UserMessage", "id": u_item_id, "content": [{"type": "text", "text": user_combined}]},
            },
        )
        ...
        if agent_reply:
            ...
            add_rec("event_msg", {"type": "task_complete", "turn_id": tid, "last_agent_message": agent_reply})

    with open(out_rollout_path, "w", encoding="utf-8", newline="\n") as fout:
        for r in records:
            fout.write(r + "\n")
